...
public class Java8ClassFile {
    ...
    public Java8ClassFile(Class clazz) throws ClassNotFoundException, IOException, IllegalClassFormatException {
        this(new ClassReader(clazz));
        this.clazz = clazz;
    }

    public Java8ClassFile(String path, String name) throws ClassNotFoundException, IOException, IllegalClassFormatException {
        this(new ClassReader(path, name));
    }

    public Java8ClassFile(InputStream in) throws IOException, IllegalClassFormatException {
        this(new ClassReader(in));
    }

    public Java8ClassFile(byte[] codes) throws IllegalClassFormatException {
        this(new ClassReader(codes));
    }

    public Java8ClassFile(ClassReader reader) throws IllegalClassFormatException {
    ...
    public static void main(String[] args) throws Exception {
        Java8ClassFile classFile = new Java8ClassFile(LinkedHashMap.class);

        //这里换成自己的class包路径
        String path = "D:\\java\\github\\spring-boot-protocol\\target\\classes\\com\\github\\netty\\protocol\\servlet";
        Map<String, Java8ClassFile> javaClassMap = new HashMap<>();
        File[] files = new File(path).listFiles();
        if (files != null) {
            for (File file : files) {
                String fileName = file.getName();
                if (fileName.endsWith(".class")) {
                    Java8ClassFile javaClassFile = new Java8ClassFile(path, fileName);
    ...
    public Java8ClassFile getSuperClassFile() throws IllegalClassFormatException, IOException, ClassNotFoundException {
        if (superClassIndex != 0) {
            return new Java8ClassFile(getSuperClass());
        }
        return null;
    }
    ...
    public static class Member {
        ...
        private Java8ClassFile classFile;
