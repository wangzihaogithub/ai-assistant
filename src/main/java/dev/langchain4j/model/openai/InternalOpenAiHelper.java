...
import com.github.aiassistant.entity.model.langchain4j.InputAudioContent;
import com.github.aiassistant.entity.model.langchain4j.OcrImageContent;
...
public class InternalOpenAiHelper {
    ...
    private static dev.ai4j.openai4j.chat.Content toOpenAiContent(Content content) {
        if (content instanceof TextContent) {
            ...
            return toOpenAiContent((VideoContent) content);
        } else if (content instanceof VideoArrayContent) {
            return toOpenAiContent((VideoArrayContent) content);
    ...
    private static dev.ai4j.openai4j.chat.Content toOpenAiContent(ImageContent content) {
        ...
        dev.ai4j.openai4j.chat.Content.Builder builder = dev.ai4j.openai4j.chat.Content.builder()
                .type(ContentType.IMAGE_URL)
                .imageUrl(ImageUrl.builder()
                        .url(toUrl(image.url(), image.mimeType(), image.base64Data()))
                        .detail(toDetail(content.detailLevel()))
                        .build());
        if (content instanceof OcrImageContent) {
            OcrImageContent ocr = ((OcrImageContent) content);
            builder.minPixels(ocr.getMinPixels());
            builder.maxPixels(ocr.getMaxPixels());
        }
        return builder.build();
    }

    private static dev.ai4j.openai4j.chat.Content toOpenAiContent(VideoContent content) {
        Video video = content.video();
        dev.ai4j.openai4j.chat.Content.Builder builder = dev.ai4j.openai4j.chat.Content.builder()
                .type(ContentType.VIDEO_URL)
                .videoUrl(new VideoUrl(toUrl(video.url(), video.mimeType(), video.base64Data())));
        return builder.build();
    }

    private static dev.ai4j.openai4j.chat.Content toOpenAiContent(VideoArrayContent content) {
        dev.ai4j.openai4j.chat.Content.Builder builder = dev.ai4j.openai4j.chat.Content.builder()
                .type(ContentType.VIDEO)
                .video(content.getArray().stream()
                        .map(e -> {
                            Video video = e.video();
                            return toUrl(video.url(), video.mimeType(), video.base64Data());
                        })
                        .collect(toList()));
        return builder.build();
    }

    private static dev.ai4j.openai4j.chat.Content toOpenAiContent(AudioContent content) {
        Audio audio = content.audio();
        String url = toUrl(audio.url(), audio.mimeType(), audio.base64Data());
        String format = null;
        if (content instanceof InputAudioContent) {
            InputAudioContent inputAudio = (InputAudioContent) content;
            format = inputAudio.getFormat();
        }
        dev.ai4j.openai4j.chat.Content.Builder builder = dev.ai4j.openai4j.chat.Content.builder()
                .type(ContentType.INPUT_AUDIO)
                .inputAudio(new InputAudio(url, format));
        return builder.build();
    }

    private static String toUrl(Object url, String mimeType, String base64Data) {
        if (url != null && !"".equals(url)) {
            return url.toString();
        }
        String row1 = "data:";
        String row3 = ";base64,";
        StringBuilder builder = new StringBuilder(row1.length() + mimeType.length() + row3.length() + base64Data.length());
        builder.append(row1);
